...
import ContructionCard from "@/src/components/Construction/Construction";
interface props {
  title?: string;
}
...
function HeaderContent({ title }: props) {
  ...
  const [constructionOpen, setContructionOpen] = useState(false);
    ...
    <>
      <header className="sticky top-0 z-40 h-16 w-full shrink-0 border-b-2 border-border bg-white px-3 pl-16 sm:px-5 sm:pl-20 lg:h-20 lg:px-6 lg:pl-6">
        <div className="max-w-7xl mx-auto flex h-full min-w-0 items-center justify-between gap-3 sm:gap-4">
          ...
          <div
            onClick={() => setContructionOpen(true)}
      ...
      {constructionOpen && (
        <ContructionCard
          setConstructionOpen={setContructionOpen}
          title="Admin"
        />
      )}
    </>
