...
function page({ sortedUsers }: props) {
  ...
  const noOfPages = Math.ceil(sortedUsers.length / usersPerPage);
            ...
            <thead>
              <tr className="border-b border-slate-100 text-left bg-slate-50/50">
                {columnNames.map((column) => (
                  <th
                    key={column}
                    className={`px-2 py-3 text-[10px] sm:px-5 sm:text-xs font-semibold text-slate-500 uppercase tracking-wider ${column === "username" || column === "Email" || column === "Company" || column === "City" ? "hidden md:table-cell" : ""} ${column === "Actions" ? "text-right" : ""}`}
                  >
                    {column}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-slate-50">
              {usersToShow.map((user) => (
                ...
                >
                  ...
                  {/* Second Column */}
                  <td className="hidden md:table-cell px-2 py-3.5 sm:px-5 text-slate-500 truncate">
                    {user.username}
                  </td>
                  {/* Third Column */}
                  <td className="hidden md:table-cell px-2 py-3.5 sm:px-5 text-slate-500 truncate">
